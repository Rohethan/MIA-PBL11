...
def list():
    ...
    <a href='/add'>Add</a><br>
<a href='/new_campus'>New Campus </a><br>
    <table border='1'>
    <tr>
...
<a href='/new_campus'>New Campus </a><br>
        <br>Make a wish<br>
        <form action='add_campus' method='GET'>
        <label>Campus Name</label><br>
        <input type='text' name='campus' required><br>
        <label>Campus ID:</label><br>
        <input type='number' name='campus_id' required><br>
        </select><br><input type='submit' value='Save'></form>"""


@app.route("/add_campus")
def addcampus():
    htmlCode = """
        Menu<br>
        <a href='/'>Homepage</a><br>
...
<a href='/new_campus'>New Campus </a><br>"""
    ...
    cur.execute("INSERT INTO Campus VALUES (%s, %s)", (int(request.values['campus_id']), str(request.values['campus'])))
    mysql.connection.commit()
    htmlCode += str(cur.rowcount) + " record inserted."
    return htmlCode
